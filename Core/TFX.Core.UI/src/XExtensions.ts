...
{
    let ar = new Array<XKeyValue<any, XArray<XArray<T>>>>();
    let ord = this.OrderBy(e => pValue(e));
    let lvlr = null;
    let lar: XKeyValue<any, XArray<XArray<T>>> | null = null;
...
Array.prototype.GetRandom = function <T>(pCount?: number): Array<T>
{
    ...
    if (pCount == null || pCount <= 0)
        pCount = this.length;
    let curidx = Math.min(pCount, ar.length);
    let rndidx = 0;
    XMath.Seed();
    while (0 !== curidx)
    {
...
{
    for (let i = 0; i < pArgments.length; i++)
        this[i] = pArgments[i];
};

Array.prototype._ComparerD = function <T>(pLeft: T, pRigh: T): number
{
    let l = this.Order(pLeft);
    let r = this.Order(pRigh);
    if (l.localeCompare)
...
Array.prototype.LPad = function <T>(pLength: number): Array<T>
{
    let ar = new XArray<T>();
    ar.length = pLength;
    let ed = this.length - 1;
    for (let i = pLength - 1; i >= 0; i--, ed--)
        ar[i] = this[ed];
    return ar;
};
...
Array.prototype.Insert = function (pIndex: number, pValue: any)
{
    ...
    for (let i = this.length - 1; i >= pIndex; i--)
        this[i] = this[i - 1];
    this[pIndex] = pValue;
};
...
Array.prototype.Contains = function (pValue: any, pStart?: number, pEnd?: number): boolean
{
    ...
    {
        let e = Math.max(pStart, <any>pEnd);
        let s = Math.min(pStart, <any>pEnd);
        if (e == s)
            return false;
        for (let i = s; i < e; i++)
        {
...
{
    let canvas = window.Canvas || (window.Canvas = document.createElement("canvas"));
    let context = canvas.getContext("2d");
    context.font = pFont;
    let metrics = context.measureText(pText);
    return metrics.width;
};
HTMLElement.prototype.Location = function (pReference: HTMLElement): XPoint
{
    let elm = <HTMLElement>this;
    let r1 = elm.getBoundingClientRect();
    let r2 = pReference.getBoundingClientRect();
...
Date.prototype.Add = function (pPart: XDatePart, pValue: number): Date
{
    ...
    let CheckRollover = function (pThis: Date)
    {
...
{
    let dstr = X.PadStart(this.getUTCDate().toString(), 2, "0") + "/" + X.PadStart(this.getUTCMonth() + 1, 2, "0") + "/" + this.getUTCFullYear().toString();
    return dstr;
}
...
{
    let dstr = this.getFullYear() + "-" + X.PadStart(this.getMonth() + 1, 2, "0") + "-" + X.PadStart(this.getDate(), 2, "0") + "T" +
...
{
    let dstr = this.getUTCFullYear() + "-" + X.PadStart(this.getUTCMonth() + 1, 2, "0") + "-" + X.PadStart(this.getUTCDate(), 2, "0") + "T" +
...
{
    let dstr = "1755-01-01T" + X.PadStart(this.getUTCHours(), 2, "0") + ":" + X.PadStart(this.getUTCMinutes().toString(), 2, "0") + ":" +
        X.PadStart(this.getUTCSeconds(), 2, "0") + "." + X.PadStart(this.getUTCMilliseconds(), 6, "0");
...
{
    let dstr = this.getUTCFullYear() + "-" + X.PadStart(this.getUTCMonth() + 1, 2, "0") + "-" + X.PadStart(this.getUTCDate(), 2, "0") + "T00:00:00.000000"
    return Date.ToDateTime(dstr, true);
}
...
Date.FromBRDate = function (pValue: string): XDateTimeResult
{
    ...
    try
    {
        if (X.IsEmpty(pValue))
            return res;
        pValue = pValue.trim();
        let strs1 = pValue.split(" ");
        if (strs1.length != 1)
            return res;
        let strs2: any = strs1[0].split("/");
        if (strs2.length != 3)
            return res;
        let d: any = new Number(strs2[0]);
        let m: any = new Number(strs2[1]);
        let y: any = new Number(strs2[2]);
        if (d < 0 || m <= 0 || y <= 1754)
            return res
        let iso = strs2[2] + "-" + strs2[1] + "-" + strs2[0] + "T00:00:00+0000";
        let dt = new Date(Date.parse(iso));
...
Date.FromBRTime = function (pValue: string): XDateTimeResult
{
    ...
    try
    {
        ...
        if (strs2.length == 2)
            strs2[2] = "00";
        let h = Number.parseInt(strs2[0]);
        let m = Number.parseInt(strs2[1]);
        let s = Number.parseInt(strs2[2]);
        if (s > 60 || m > 60 || h > 23)
            return res;
        let iso = "1755-01-01T" + strs2[0] + ":" + strs2[1] + ":" + strs2[2] + "+0000";
        let dt = new Date(Date.parse(iso));
...
Date.FromDate = function (pValue: string): XDateTimeResult
{
    ...
    try
    {
        ...
        if (strs2.length != 3)
            return res;
        let y = Number.parseInt(strs2[0]);
        let m = Number.parseInt(strs2[1]);
        let d = Number.parseInt(strs2[2]);
        if (d < 0 || d > 32 || m < 0 || m > 11 || y <= 1754)
            return res;
        let iso = strs2[0] + "-" + strs2[1] + "-" + strs2[2] + "T00:00:00+0000";
        let dt = new Date(Date.parse(iso));
        if (dt.getUTCDate() != d || dt.getUTCMonth() + 1 != m || dt.getUTCFullYear() != y)
            return res;
        res.IsValid = true;
...
Date.FromDateTime = function (pValue: string): XDateTimeResult
{
    ...
    try
    {
        ...
        if (d < 0 || d > 31 || m < 0 || m > 12 || y <= 1754)
            return res;
        let strs3: any = strs1[1].split(":");
        if (strs3.length < 2 || strs3.length > 3)
            return res;
        if (strs2.length == 2)
            strs3[2] = "00";
        let h = Number.parseInt(strs3[0]);
        let n = Number.parseInt(strs3[1]);
        let s = Number.parseInt(strs3[2]);
        if (s > 60 || n > 60 || h > 23)
            return res;
        let iso = "" + strs2[0] + "-" + strs2[1] + "-" + strs2[2] + "T" + strs3[0] + ":" + strs3[1] + ":" + strs3[2] + "+0000";
        let dt = new Date(Date.parse(iso));
...
Date.FromBRDateTime = function (pValue: string): XDateTimeResult
{
    ...
    try
    {
        ...
        if (strs2.length != 3)
            return res;
        let d = Number.parseInt(strs2[0]);
        let m = Number.parseInt(strs2[1]);
        let y = Number.parseInt(strs2[2]);
        if (d < 0 || m <= 0 || y <= 1754)
            return res;
        let strs3: any = strs1[1].split(":");
        if (strs3.length < 2 || strs3.length > 3)
        ...
        if (s > 60 || n > 60 || h > 23)
            return res;
        let iso = strs2[2] + "-" + strs2[1] + "-" + strs2[0] + "T" + strs3[0] + ":" + strs3[1] + ":" + strs3[2] + "+0000";
        let dt = new Date(Date.parse(iso));
        if (dt.getUTCDate() != d || dt.getUTCMonth() + 1 != m || dt.getUTCFullYear() != y || dt.getUTCHours() != h || dt.getUTCMinutes() != n || dt.getUTCSeconds() != s)
            return res;
        res.IsValid = true;
...
class X
{
    ...
    static Split(pValue: string, pSeparetor: string): Array<string>
    {
        if (X.IsEmpty(pValue))
            return [];
        let prts = pValue.split(pSeparetor);
...
class XCall
{
    ...
    {
        if (pElement.Method != null)
            for (let vle in pElement.Method)
