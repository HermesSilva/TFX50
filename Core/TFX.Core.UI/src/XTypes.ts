...
class XFieldComposer
{
    private container: HTMLElement
    private fieldDefs: { name: string; mask?: string }[]
    private selectedIndex = 0
    private listElement?: HTMLUListElement

    constructor(container: HTMLElement, fields: string[])
    {
        this.container = container
        this.fieldDefs = fields.map(f =>
        {
            const parts = f.split(';')
            return parts.length > 1
                ? { name: parts[0], mask: parts[1] }
                : { name: parts[0] }
        })
        container.tabIndex = 0
        container.classList.add('x-field-composer')
        //container.addEventListener('focus', () => this.showFieldList(), false)
        container.addEventListener('keydown', e => this.onKeyDown(e), false)
        container.addEventListener('keydown', e => this.onDocumentKeyDown(e), false)
        this.hideList()
        this.addEditor(this.fieldDefs[0])
    }
    ...
    {
        if (e.key === 'Enter' && this.listElement)
        {
            e.preventDefault()
            this.selectField(this.selectedIndex)
            return
        }
        if (!this.listElement)
            return
        if (e.key === 'ArrowDown')
        {
            ...
            if (e.key === 'ArrowUp')
            {
                e.preventDefault(); this.changeSelection(-1)
            }
    }
    ...
    {
        if (this.listElement) this.hideList()
        const used = new Set(
            Array.from(this.container.querySelectorAll('.field-editor'))
                .map(el => el.getAttribute('data-name')!)
        )
        const available = this.fieldDefs.filter(d => !used.has(d.name))
        ...
        available.forEach((def, i) =>
        {
            ...
            li.addEventListener('click', () => this.selectField(i), false)
            ul.appendChild(li)
        })
    ...
    {
        const items = Array.from(this.listElement!.children) as HTMLElement[]
    ...
    private addEditor(def: { name: string; mask?: string })
    {
        const editor = document.createElement('div')
        editor.className = 'field-editor'
        editor.setAttribute('data-name', def.name)
        ...
        input.type = 'text'
        input.tabIndex = 0
        input.addEventListener('input', () => this.onInput(input, def.mask), false)

        const remove = document.createElement('span')
        remove.className = 'remove'
        remove.innerHTML = 'X'
        remove.addEventListener('click', () => editor.remove(), false)
    ...
    private onInput(input: HTMLInputElement, mask?: string)
    {
        if (mask)
        {
            const digits = input.value.replace(/\D/g, '')
            input.value = this.applyMask(digits, mask)
        }
