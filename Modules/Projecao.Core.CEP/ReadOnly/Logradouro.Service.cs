...
using System.Text;
using Projecao.Core.CEP.ReadOnly.Rules;
using Projecao.Core.CEP.ReadOnly;

namespace Projecao.Core.CEP.ReadOnly
{
    [XGuid("F7E6E0E6-49C5-41F1-AF4F-C54123E24C1E", typeof(ILogradouroService))]
    public class LogradouroService : XService, ILogradouroService
    {
        ...
        public IQueryable<LogradouroTuple> ExecuteQuery(LogradouroFilter pFilter)
        {
            ...
            var query = from CEPxLogradouro in ctx.CEPxLogradouro
                        ...
                        join CEPxBairro in ctx.CEPxBairro on CEPxLocalidade.CEPxLocalidadeID equals CEPxBairro.CEPxLocalidadeID
                        
                        select new {CEPxLogradouro, CEPxLocalidade, CEPxUF, CEPxBairro};
            query = _INFRule.GetWhere(query);


            if (pFilter != null)
            {
                if (!pFilter.Logradouro.IsEmpty())
                    query = query.Where(q => EF.Functions.Like(q.CEPxLogradouro.Nome, "%"+pFilter.Logradouro+"%"));
            }
            ...
            var qry = query.Select(q => new LogradouroTuple(q.CEPxLocalidade.Nome,
