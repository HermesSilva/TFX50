...
namespace Tootega.Core.ERP.Profissional.Test
{
    public class ProfissionalTest : TestSetup
    {
        public ProfissionalTest(XServerSidePrepare pPrepare)
               :base(pPrepare)
        {
        }
        ...
        {
            var obj = PrepareClient("http://localhost:7000/Profissional/Flush", null);
        ...
        {
            var obj = PrepareClient("http://localhost:7000/Profissional/Flush", new ProfissionalDataSet());
            var ret = await DoCall<XResponse>(obj);
        ...
        {
            var dst = new ProfissionalDataSet();
            dst.Tuples.Add(new ProfissionalTuple());
            var obj = PrepareClient("http://localhost:7000/Profissional/Flush", dst);
            var ret = await DoCall<XResponse>(obj);
            Assert.Equal(404, ret.Status);
            if (ret.Data is JObject jo)
            {
