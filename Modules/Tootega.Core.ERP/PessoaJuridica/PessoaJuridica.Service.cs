...
using System.Text;
using Tootega.Core.ERP.PessoaJuridica.Rules;
using Tootega.Core.ERP.PessoaJuridica;
using Tootega.Core.ERP.Pessoa;

namespace Tootega.Core.ERP.PessoaJuridica
{
    [XGuid("7B3F7668-C970-4650-A88A-C681AA867590", typeof(IPessoaJuridicaService))]
    public class PessoaJuridicaService : XService, IPessoaJuridicaService
    {
        ...
        public abstract class BaseINFPessoaJuridicaServiceRule : XServiceINFRule<PessoaJuridicaService, PessoaJuridicaTuple>
        {
            public BaseINFPessoaJuridicaServiceRule(PessoaJuridicaService pService)
                : base(pService)
            {
            }
        ...
        public PessoaJuridicaService(ILogger<XService> pLogger, DBContext pContext)
               :base(pLogger)
        {
            Rule = new PessoaJuridicaRule(this);
            _INFRule = new INFPessoaJuridicaServiceRule(this);
            Context = pContext;
            _INFRule.SetContext(Context);
        }

        internal XIServiceRule<PessoaJuridicaTuple, PessoaJuridicaTuple> Rule;
        private INFPessoaJuridicaServiceRule _INFRule;
        ...
        public IQueryable<PessoaJuridicaTuple> ExecuteQuery(PessoaJuridicaFilter pFilter, Boolean pFull)
        {
            var ctx = Context;
            var query =  from ERPxPessoaJuridica in ctx.ERPxPessoaJuridica
                        join CORxPessoa in ctx.CORxPessoa on ERPxPessoaJuridica.ERPxPessoaJuridicaID equals CORxPessoa.CORxPessoaID
                        select new { ERPxPessoaJuridica, CORxPessoa };
;
            ...
            var qry = query.Select(q => new PessoaJuridicaTuple(q.ERPxPessoaJuridica.RazaoSocial,
        ...
        public PessoaJuridicaDataSet Execute(PessoaJuridicaFilter pFilter, Boolean pFull)
        {
        ...
        public object Flush(PessoaJuridicaDataSet pDataSet)
        {
            ...
            using (var ctx = scope.ServiceProvider.GetRequiredService<DBContext>())
            {
                ...
                SetPessoaJuridicaValues(ctx, pDataSet);
                ctx.SaveChanges();
        ...
        private void SetPessoaJuridicaValues(DBContext ctx, PessoaJuridicaDataSet pDataSet)
        {
            if (pDataSet == null || pDataSet.Tuples == null)
                return;
            foreach (PessoaJuridicaTuple stpl in pDataSet.Tuples)
            {
