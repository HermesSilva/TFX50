...
## Restrições de pastas
- Ao ser espeficado uma pasta deve ser limita apenas a ela a busca e avalações de arquivos
- Se for cita uma classe com compote em TypeScipt de ser usado apenas a basta Core\TFX.Core.UI
- Pasta exclusiva de UI Core\TFX.Core.UI
- Havendo alterações de UI deve ser compilado a solução Core\TFX.Core.UI\TFX.Core.UI.sln e não a principal.
